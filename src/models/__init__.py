...
def infer_model_type_from_state_dict(state_dict: dict[str, torch.Tensor]) -> str:
    """
    Identifies the model architecture based on the keys present in the state dictionary.
    Handles prefixes generated by torch.compile (_orig_mod.).
    """
    # Clean keys if they come from torch.compile (_orig_mod. prefix)
    keys = [k.replace("_orig_mod.", "") for k in state_dict.keys()]

    if any(k.startswith("entity_encoder.") for k in keys):
        return "inn_ours_mlp"
    if any(k.startswith("layer.W_in.") for k in keys):
        return "inn_compgcn"
    if (
        any(k.startswith("entity_emb.") for k in keys)
        and not any(k.startswith("layer.") for k in keys)
        and "net.layers.0.W.weight" not in keys
    ):
        # Distinguish between RotatE and TransE by comparing embedding dimensions
        rel_center_key = "rel_center.weight"
        ent_center_key = "entity_emb.center.weight"

        # Map back to original keys to retrieve weight shapes
        orig_rel_center = next(
            (k for k in state_dict if k.replace("_orig_mod.", "") == rel_center_key),
            None,
        )
        orig_ent_center = next(
            (k for k in state_dict if k.replace("_orig_mod.", "") == ent_center_key),
            None,
        )

        if orig_rel_center and orig_ent_center:
            # TransE relations have the same dimensionality as entity centers
            if (
                state_dict[orig_rel_center].shape[1]
                == state_dict[orig_ent_center].shape[1]
            ):
                return "inn_transe"
        return "inn_rotate"
    if any(k.startswith("entity_emb.") for k in keys):
        return "inn_lightgcn"
    return "inn_ours_mlp"
...
def logsumexp_loss(
    ...
    """
    LogSumExp loss (Soft-margin alternative).
    """
...
def infonce_loss(
    ...
    """
    InfoNCE loss (Contrastive learning). alpha acts as inverse temperature (1/tau).
    """
